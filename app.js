...
const firebaseConfig = {
  apiKey: "AIzaSyCu6otcTpNOcmUBUc9bPFkpEZdAbSz3oFY",
  authDomain: "kupameshutefet-6d477.firebaseapp.com",
  projectId: "kupameshutefet-6d477",
  storageBucket: "kupameshutefet-6d477.firebasestorage.app",
  messagingSenderId: "710239793776",
  appId: "1:710239793776:web:59dd43467b2005e73cbab5"
};
firebase.initializeApp(firebaseConfig);
const db = firebase.firestore();
const auth = firebase.auth();

// Top-level collections (not per-user)
...
let config = {
  ...
  partner2Name: null,
  accountType: "couple"  // "couple" | "solo"
};
...
function p2() { return config.partner2Name || null; }
function soloMode() { return config.accountType === "solo"; }

function updatePartnerNamesInUI() {
  const n1 = p1(), n2 = p2(), isSolo = soloMode();
  ACCOUNTS = isSolo ? [n1, "מזומן"] : [n1, n2, "מזומן"];
  BALANCE_ACCOUNTS = isSolo ? [n1, "מזומן", "חיסכון"] : [n1, n2, "מזומן", "חיסכון"];
  ...
  const sl2 = document.getElementById("stat-label-p2"); if (sl2) sl2.textContent = n2 ? `שילם/ה ${n2}` : "";

  // Auth tagline
  const tagline = document.querySelector(".auth-tagline");
  if (tagline) tagline.textContent = isSolo ? "ניהול כספים חכם" : "ניהול כספים לזוגות";
  ...
  // Show/hide couple-only elements
  document.querySelectorAll("[data-couple-only]").forEach((el) => {
    el.style.display = isSolo ? "none" : "";
  });

  // Report partner filter labels
  const pfp1 = document.getElementById("partner-filter-p1");
  const pfp2 = document.getElementById("partner-filter-p2");
  if (pfp1) pfp1.textContent = n1;
  if (pfp2 && n2) pfp2.textContent = n2;
}
...
let registerMode = "couple";
document.querySelectorAll(".mode-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    registerMode = btn.dataset.mode;
    document.querySelectorAll(".mode-btn").forEach((b) => b.classList.toggle("active", b.dataset.mode === registerMode));
    const p2Field = document.getElementById("reg-p2-field");
    const p2Input = document.getElementById("reg-partner2");
    const p1Label = document.getElementById("reg-p1-label");
    const submitBtn = document.getElementById("reg-submit-btn");
    if (registerMode === "solo") {
      p2Field.style.display = "none";
      p2Input.required = false;
      p1Label.textContent = "השם שלך 👤";
      submitBtn.textContent = "יצירת הקופה שלי 🎉";
    } else {
      p2Field.style.display = "";
      p2Input.required = true;
      p1Label.textContent = "שם בן/בת הזוג הראשון/ה 👤";
      submitBtn.textContent = "יצירת הקופה שלנו 🎉";
    }
    clearAuthError();
  });
});
...
document.getElementById("register-form").addEventListener("submit", (e) => {
  ...
  const p2Name = registerMode === "couple" ? document.getElementById("reg-partner2").value.trim() : null;
...
function renderReportsView() {
  ...
  if (!soloMode() && reportPartnerFilter !== "all") {
    const filterName = reportPartnerFilter === "p1" ? p1() : p2();
    periodExp = periodExp.filter((e) => e.paidBy === filterName);
    periodInc = periodInc.filter((e) => e.account === filterName);
  }
...
document.getElementById("expense-form").addEventListener("submit", (e) => {
  ...
  if (modalType === "income") {
    ...
    expensesRef.add({ amount, description, category, paidBy: account, source: "web", date: firebase.firestore.Timestamp.now() }).then(() => {
      if (account === "מזומן") showToast("מס הכנסה בדרך 😅💵");
      else if (!soloMode() && account === p2()) showToast(`הופההה ${p2()} שילמה מי היה מאמין 😂`);
      else if (!soloMode() && account === p1()) showToast(`סוף סוף ${p1()} משלם 😎`);
      resetAndClose();
...
async function loadAdminCodes() {
  ...
  listEl.innerHTML = `<p class="admin-loading">טוען קודים...</p>`;
  try {
    const snap = await accessCodesRef.orderBy("createdAt", "desc").get();
    if (snap.empty) { listEl.innerHTML = `<p class="admin-empty">אין קודי גישה עדיין — צור קוד חדש למעלה</p>`; return; }
    listEl.innerHTML = snap.docs.map((doc) => {
      const d = doc.data();
      const date = d.createdAt ? d.createdAt.toDate().toLocaleDateString("he-IL") : "—";
      const isUsed = d.used;
      return `<div class="admin-code-row">
        <div>
          <span class="admin-code-text">${escapeHtml(doc.id)}</span>
          ${d.note ? `<span class="admin-code-note">${escapeHtml(d.note)}</span>` : ""}
        </div>
        <div style="text-align:start">
          <span class="admin-status-badge ${isUsed ? "badge-used" : "badge-active"}">${isUsed ? "שומש" : "פנוי"}</span>
          ${isUsed ? `<div class="admin-code-usedby">↳ ${escapeHtml(d.usedBy || "")}</div>` : ""}
          <div class="admin-code-date">${date}</div>
        </div>
        ${!isUsed ? `<button class="admin-del-btn" onclick="deleteCode('${doc.id}', this)">🗑</button>` : ""}
      </div>`;
    }).join("");
  } catch(e) {
    listEl.innerHTML = `<p class="admin-empty" style="color:red">שגיאה: ${e.message}</p>`;
...
async function generateAndSaveCode() {
  const note = document.getElementById("admin-code-note").value.trim();
  const code = generateCode();
  try {
    await accessCodesRef.doc(code).set({
      used: false, note, createdAt: firebase.firestore.Timestamp.now()
    });
    document.getElementById("admin-code-note").value = "";
    await loadAdminCodes();
    // Show the code prominently
    showToast(`קוד חדש: ${code} 📋`);
  } catch(e) { alert("שגיאה: " + e.message); }
}

async function deleteCode(code, btn) {
  if (!confirm(`למחוק את הקוד ${code}?`)) return;
  btn.disabled = true;
  try {
    await accessCodesRef.doc(code).delete();
    await loadAdminCodes();
  } catch(e) { btn.disabled = false; alert("שגיאה: " + e.message); }
}
...
document.getElementById("admin-generate-code-btn").addEventListener("click", generateAndSaveCode);
